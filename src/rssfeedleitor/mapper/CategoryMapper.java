...
public interface CategoryMapper {
	...
	@Select("SELECT c.id, c.title, (SELECT count(*) FROM feed f INNER JOIN channel ch ON(f.channel_id = ch.id) WHERE f.visualized = 0 AND ch.category_id = c.id) as unread FROM category c WHERE c.user_id = #{user.id}")
	@Results(value = {
